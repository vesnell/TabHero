...
public class MyLongClickAdapterToLock implements AdapterView.OnLongClickListener {
	...
	public boolean onLongClick(View v) {
		...
		if(!lock) {
			...
			btnLock.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					...
					lock = true;
					Toast.makeText(context.getApplicationContext(), R.string.lockOn, Toast.LENGTH_LONG).show();
			...
			btnUnLock.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					...
					lock = false;
					Toast.makeText(context.getApplicationContext(), R.string.lockOff, Toast.LENGTH_LONG).show();
					btnUnLock.setVisibility(View.GONE);
				}
			});
		}
		...
		new Handler().postDelayed(lockBtnsRunnable, 3000);
		return false;
	}
